"""Structural, synthetic/config-only checks for readiness-audit allowlist v1.0.2."""
...
ALLOWLIST_PATH = ROOT / "configs/thesis_readiness_audit_v1_0_2_allowlist.yaml"
...
class ThesisReadinessAuditAllowlistTests(unittest.TestCase):
    ...
    def test_preparation_does_not_self_authorize_review_or_audit(self) -> None:
        ...
        self.assertEqual(self.control["version"], "1.0.2")
        self.assertEqual(
    ...
    def test_successor_pins_failed_review_and_preserves_ancestors(self) -> None:
        ...
        self.assertEqual(
            predecessor["path"],
            "configs/thesis_readiness_audit_v1_0_1_allowlist.yaml",
        )
        self.assertEqual(predecessor["version"], "1.0.1")
        self.assertEqual(
            predecessor["git_commit"],
            "da4d76913930240e4f04190df8a4b0cc322fa74b",
        )
        ...
        self.assertEqual(
            review["git_commit"],
            "9ba01a06c066153af0681a0351cc918bee020697",
        )
    ...
    def test_review_scope_is_narrow_non_analytical_and_existing(self) -> None:
        ...
        self.assertIn(
            "tests/test_thesis_readiness_audit_allowlist_v1_0_2.py",
            review,
        )
        executor_paths = {
            path
    ...
    def test_forbidden_operations_limits_outputs_and_stop_fail_closed(self) -> None:
        ...
        required_forbidden = {
            ...
            "render_review_subject_without_bounded_exact_closed_line_range",
            "combine_review_subject_read_with_another_content_read",
        }
        ...
        self.assertEqual(limits["max_rendered_lines_per_command"], 200)
        self.assertEqual(limits["max_review_subject_lines_rendered_per_command"], 120)
    ...
    def test_review_subject_requires_complete_bounded_chunk_sequence(self) -> None:
        protocol = self.payload["review_subject_read_protocol"]
        subject = "configs/thesis_readiness_audit_v1_0_2_allowlist.yaml"
        self.assertEqual(protocol["exact_path"], subject)
        self.assertEqual(protocol["applies_to_contexts"], ["independent_review"])
        self.assertTrue(protocol["mandatory_before_substantive_review"])
        self.assertFalse(protocol["full_file_render_in_one_command_allowed"])
        self.assertFalse(
            protocol["generic_review_content_read_operation_allowed_for_subject"]
        )
        self.assertIn(subject, self.scope["exact_content_read_allowlist_for_review"])

        operations = set(self.payload["allowed_operations"]["independent_review"])
        probe = protocol["line_count_probe"]
        self.assertIn(probe["operation"], operations)
        self.assertEqual(probe["exact_command"], ["wc", "-l", subject])
        self.assertEqual(probe["content_lines_rendered"], 0)

        chunk = protocol["bounded_chunk_read"]
        self.assertIn(chunk["operation"], operations)
        self.assertEqual(
            chunk["exact_command_template"],
            ["sed", "-n", "{start},{end}p", subject],
        )
        self.assertTrue(chunk["exact_closed_line_range_required"])
        self.assertTrue(chunk["commands_may_not_be_combined"])
        self.assertTrue(chunk["one_content_file_per_command"])
        self.assertEqual(chunk["end_must_not_exceed_start_plus"], 119)

        limits = self.payload["output_limits"]
        subject_limit = chunk["maximum_rendered_lines_per_command"]
        self.assertEqual(subject_limit, 120)
        self.assertEqual(
            subject_limit,
            limits["max_review_subject_lines_rendered_per_command"],
        )
        self.assertLess(subject_limit, limits["max_rendered_lines_per_command"])
        self.assertLessEqual(limits["max_rendered_lines_per_command"], 240)

        constraints = self.payload["operation_constraints"]
        generic = constraints["read_exact_review_allowlisted_file"]
        self.assertEqual(generic["review_subject_path_excluded"], subject)
        bounded = constraints[chunk["operation"]]
        self.assertEqual(bounded["exact_path"], subject)
        self.assertEqual(
            bounded["maximum_rendered_lines_per_command"], subject_limit
        )
        self.assertTrue(bounded["exact_closed_line_range_required"])
        self.assertTrue(bounded["single_content_file_per_command"])

        with ALLOWLIST_PATH.open("r", encoding="utf-8") as handle:
            line_count = sum(1 for _ in handle)
        ranges = [
            (start, min(start + subject_limit - 1, line_count))
            for start in range(1, line_count + 1, subject_limit)
        ]
        self.assertGreater(len(ranges), 1)
        self.assertEqual(ranges[0][0], 1)
        self.assertEqual(ranges[-1][1], line_count)
        for index, (start, end) in enumerate(ranges):
            self.assertLessEqual(end - start + 1, subject_limit)
            if index:
                self.assertEqual(start, ranges[index - 1][1] + 1)

        guard = protocol["pre_command_guard"]
        self.assertTrue(guard["projected_rendered_lines_must_not_exceed_subject_limit"])
        self.assertTrue(guard["projected_rendered_lines_must_not_exceed_global_limit"])
        self.assertTrue(guard["stop_before_execution_if_either_limit_would_be_exceeded"])
        self.assertEqual(guard["stop_route"], "review_process_nonconformance")
        self.assertEqual(
            self.payload["stop_policy"]["mandatory_trigger_routes"][
                "independent_review"
            ]["output_cap_exceeded_without_content_exposure"],
            guard["stop_route"],
        )

    def test_review_commit_is_exactly_limited_to_two_verdict_outputs(self) -> None:
        ...
        self.assertEqual(constraint["allowed_contexts"], ["independent_review"])
        self.assertEqual(
            constraint["exact_paths_reference"],
            "audit_scope.exact_write_allowlist_for_review",
        )
        self.assertEqual(constraint["exact_path_count"], 2)
    ...
    def test_every_mandatory_stop_route_is_permitted_and_exact(self) -> None:
        ...
        self.assertEqual(
            set(triggers),
            {"independent_review", "audit_executor_after_review_pass"},
        )
    ...
    def test_audit_writes_only_new_exact_outputs(self) -> None:
        ...
        self.assertEqual(
            outputs,
            [
                "configs/thesis_readiness_audit_v1_0_2_result.yaml",
                "docs/14_thesis_readiness_audit_v1_0_2.md",
                "tests/test_thesis_readiness_audit_boundary_v1_0_2.py",
            ],
        )
        ...
        self.assertEqual(
            incident_outputs,
            [
                "configs/thesis_readiness_audit_access_incident_v1_0_2.yaml",
                "docs/13_1_thesis_readiness_audit_access_incident_v1_0_2.md",
            ],
        )
